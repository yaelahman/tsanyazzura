...
@section('content')
                        ...
                        <tbody>
                            @foreach ($aspirasi as $index => $row)
                                <tr>
                                    ...
                                    <td>{{ $row->pesan ?? '-' }}</td>
                                    <td>
                                        <button data-bs-toggle="modal" data-bs-target="#modalBalasan"
                                            data-id="{{ $row->id }}"
                                            onclick="let id = this.getAttribute('data-id'); $('#id-aspirasi').val(id); $('#balasan').val(this.getAttribute('data-balasan'))"
                                            data-balasan="{{ $row->balasan }}"
    ...
    <div class="modal fade border-primary" id="modalBalasan" tabindex="-1" role="dialog"
        aria-labelledby="modalBalasanLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" onclick="$('#form-balasan').submit(); $('#form-balasan').reset()"
                        class="btn btn-primary" style="font-size: 12px !important;">Save</button>
                </div>
            </div>
        </div>
    </div>
