                    ...
                    <table id="datatable1" class="table user-table no-wrap">
                        ...
                        <tbody>
                            @foreach ($aspirasi as $index => $row)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $row->prodi ? $row->prodi->name : '-' }}</td>
                                    <td>{{ $row->angkatan ?? '-' }}</td>
                                    <td>{{ $row->pesan ?? '-' }}</td>
                                    @if($row->balasan == null) 
                                        <td>
                                            <button data-bs-toggle="modal" data-bs-target="#modalBalasan" data-id="{{ $row->id }}" onclick="let id = this.getAttribute('data-id'); $('#id-aspirasi').val(id)"
                ...
                <div class="modal-body">
                    <form action="{{ route('aspirasi.balas') }}" method="POSt" id="form-balasan">
                        ...
                        <div class="form-group mb-2">
                            ...
                            <textarea placeholder="balasan" name="balasan" class="form-control" required></textarea>
                        </div>
                    </form>
                </div>
...
@section('script')
    <script type="text/javascript">
        $('.button-delete').click(function() {
            ...
            form.attr('action', "{{ url('/admin/visi-misi') }}/" + id);
