                ...
                <table id="datatable1" class="table user-table no-wrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>
                                <a class="btn btn-primary btn-sm btn-modal-image" href="{{ asset('tim/' . $row->image) }}" target="_blank">
                                    Lihat
                                </a>
                            </td>
                            <td>{{ $row->name }}</td>
                            <td>{{ $row->jabatan }}</td>
                            <td>
                                <button class="button-status btn btn-sm btn-{{ $row->is_utama ? 'success' : 'danger' }} btn-sm btn-flat mr-2" data-id="{{ $row->id }}" style="color: white">
                                    {{ $row->is_utama ? 'Ya' : 'Tidak' }}
                                </button>
                            </td>
                            <td>
                                <a href="{{ route('tim.edit', ['tim' => $row->id]) }}" class="btn btn-warning btn-sm btn-flat mr-2"><i class="fas fa-pencil-alt"></i>
                                    Edit</a>
                                <button class="button-delete btn btn-danger btn-sm btn-flat mr-2" data-id="{{ $row->id }}" style="color: white"><i class="fas fa-trash-alt"></i> Delete
                                </button>
                            </td>
                        </tr>
                        @endforeach
